...
<div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory">
        {PUJAS.filter((p) => p.popular).map((p) => (
...
<h3 className="mt-2 text-[11px] leading-4 font-medium md:mt-4 md:text-sm md:leading-6">
  ...
  <h2 className="mt-3 font-display text-4xl md:text-5xl">
    Find the Perfect
    <span className="text-gradient-gold"> Puja for Every Occasion</span>
  </h2>
...
<p className="mt-4 flex-1 line-clamp-3 text-sm leading-7 text-muted-foreground">              {p.tagline}
            </p>

            <div className="mt-7 border-t border-gold/10 pt-6">
              <span className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-saffron px-5 py-3 text-sm font-semibold text-primary-foreground transition-all duration-300 group-hover:scale-[1.02] group-hover:shadow-gold">
               Book Puja 
  ...
  <div className="container-luxe relative">
    ...
    <h2 className="mt-4 font-display text-4xl md:text-6xl">
     Need Guidance for the Right Puja?
      <span className="text-gradient-gold"> Puja You Need?</span>
    </h2>

    <p className="mx-auto mt-6 max-w-2xl text-lg text-cream/70">
      Talk directly with our experienced Pandit Ji and get free guidance before
      booking your puja.
    </p>
